...
naive <- dat %>%
  ungroup() %>%
  dplyr::select(true, site, replicate, richness_sim, samples_sim, events_sim, nbinom_prob_sim) %>%
...
plot1b <- ggplot(data = plot.df, aes(x=richness_sim, y=diff, fill = method, color = method)) +
  geom_boxplot(fatten = .75) +
  geom_hline(yintercept = 0) +
  facet_wrap(~events_sim) +
  scale_color_manual(values=c("#4B4B61", "#49585F", "#564444", "#8C594A", "#73507d", "grey50")) +
  scale_fill_manual(values=c("#0000FF", "#00B7FF", "#A50000", "#FF9D81", "#a33ac2", "grey")) +
  theme_classic(base_size = 16) +
  labs(x="", y = "Difference from truth", fill = "Estimator") +
  guides(color="none") +
  theme(axis.text.x = element_blank())
...
plot2 <- ggplot(data = plot.df, aes(x=richness_sim, y=precision, fill = method, color = method)) +
  geom_boxplot() +
  facet_wrap(~events_sim) +
  scale_color_manual(values=c("steelblue4", "steelblue", "red4", "red3", "purple3"), ) +
  scale_fill_manual(values=c("steelblue2", "steelblue1", "red2", "red1", "purple1")) +
  theme_classic(base_size = 16) +
...
plot2b <- ggplot(data = plot.df, aes(x=richness_sim, y=precision, fill = method, color = method)) +
  geom_boxplot(fatten = .75) +
  facet_wrap(~events_sim) +
  scale_color_manual(values=c("#4B4B61", "#49585F", "#564444", "#8C594A","#73507d")) +
  scale_fill_manual(values=c("#0000FF", "#00B7FF", "#A50000", "#FF9D81","#a33ac2")) +
  theme_classic(base_size = 16) +
  labs(x="", y="Error Range", fill = "Estimator") +
  theme(axis.text.x = element_blank(), legend.position="none", strip.text.x = element_blank())
...
panel_plot1 <- ggarrange(plot1, plot2, plot3, ncol = 1, common.legend = TRUE, legend="right", labels = c("A","B","C"), hjust = -2.25, font.label = list(face = "bold", color = "black"), align = "v")

panel_plot1b <- ggarrange(plot1b, plot2b, plot3b, ncol = 1, common.legend = TRUE, legend="right", labels = c("A","B","C"), hjust = -2.25, font.label = list(face = "bold", color = "black"), align = "v")
