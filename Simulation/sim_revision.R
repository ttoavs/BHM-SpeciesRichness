  ...
  for(j in samples_vec) {
    for(z in sampling_event_vec) {
      for(t in nbinom_size) {
        ...
        CMAX <- list()
        for(f in 1:site_num){
          CMAX[[f]] <- estimateD(replicated_samples[[f]], datatype = "incidence_raw", q = 0)
          CMAX[[f]]$replicate <- seq(1,replicates, 1)
        }
        CMAX <- bind_rows(CMAX, .id = "site")
        
        CMAX$method <- "CMAX"
        
        CMAX <- CMAX %>%
          dplyr::select(qD, site, replicate, qD.UCL, qD.LCL, method) %>%
          rename(est = qD, upper95 = qD.UCL, lower95 = qD.LCL) %>%
          mutate(sd = (upper95-lower95)/3.92)
        ...
        dat_all_df <- rbind(MM_est_df, NE_est_df, chao_est_df, jack_est_df, Naive_est_df, CMAX)
...
# Save output
saveRDS(dat_final, "data/Created/sim_revision_data.RDS")
